        ...
        <div class="flex flex-col gap-6 p-6 w-full mx-auto bg-slate-50/50 min-h-screen">
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.03)] overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm text-slate-700">
                        <thead class="bg-slate-50/80 text-xs text-slate-500 uppercase tracking-wider border-b border-slate-100">
                            <tr>
                                ...
                                <th class="px-6 py-4 font-semibold">Classification Name</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <!-- PATCH 1: Explicitly check if state === 0 for the inactive row styling -->
                            <tr v-for="classification in classifications.data" :key="classification.id" class="transition-colors hover:bg-slate-50/50" :class="{'opacity-60 bg-slate-50/30': classification.state === 0}">
                                <td class="px-6 py-4 font-mono text-xs text-slate-500">{{ classification.id }}</td>
                                <td class="px-6 py-4 font-medium text-slate-800">{{ classification.name }}</td>
                                <!-- <td class="px-6 py-4 text-slate-500">
                                    {{ classification.description || 'No description provided.' }}
                                </td> -->
                                <!-- Status Column -->
                                <td class="px-6 py-4">
                                    <!-- PATCH 2: Explicitly check if state === 1 for the Active badge -->
                                    <span v-if="classification.state === 1" class="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-100">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex justify-center gap-2">
                                        ...
                                        <!-- PATCH 3: Explicitly check if state === 1 to show the delete button -->
                                        <button
                    ...
                    <!-- Empty State -->
                    <div v-if="classifications.data.length === 0" class="py-16 text-center">
                ...
                <!-- Pagination -->
                <div v-if="classifications.last_page > 1" class="flex items-center justify-between border-t border-slate-100 bg-slate-50/50 px-6 py-4">
                    ...
                    <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl border border-slate-100">
                        ...
                        <form @submit.prevent="submitForm" class="space-y-5">
                            ...
                            <!-- <div>
                                ...
                                ></textarea>
                                <p v-if="form.errors.description" class="mt-1.5 text-xs text-red-500">{{ form.errors.description }}</p>
                            </div> -->
