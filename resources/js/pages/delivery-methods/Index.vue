...
const clearSearch = () => {
    searchTerm.value = '';
    router.get(route('delivery-methods.index'));
};
...
const breadcrumbs: BreadcrumbItem[] = [
    {
        title: 'Delivery Methods Management',
...
// Open modal for new delivery method
...
function openEditDialog(method: any) {
    ...
    form.delivery_method_name = method.delivery_method_name;
    form.state = method.state;
    showDialog.value = true;
}

// Deactivate modal (Soft Delete)
function openDeleteModal(method: { id: number; delivery_method_name: string }) {
    itemToDeleteUrl.value = route('delivery-methods.destroy', method.id);
    itemToDeleteName.value = method.delivery_method_name;
    showDeleteModal.value = true;
}

// Quick Reactivate Action
function reactivateMethod(method: any) {
    router.put(route('delivery-methods.update', method.id), {
...
function submitForm() {
    if (isEdit.value && editingId.value !== null) {
        form.put(route('delivery-methods.update', editingId.value), {
            onSuccess: () => {
        ...
        form.post(route('delivery-methods.store'), {
            onSuccess: () => {
                showDialog.value = false;
                form.reset();
            },
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <!-- Main Wrapper: Changed from bg-slate-50/50 to a solid bg-slate-100 for better contrast -->
        <div class="flex flex-col gap-6 p-6 w-full mx-auto bg-slate-100 min-h-screen">

            <!-- Header & Actions Toolbar -->
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Delivery Methods</h1>
                    <p class="text-sm text-slate-500 mt-1">Manage physical and digital document transport options.</p>
                </div>
            ...
            <!-- Table Card: Darkened border to border-slate-200 and increased shadow -->
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm text-slate-700">
                        <!-- Header: Changed to solid bg-slate-100 and border-slate-200, tightened padding -->
                        <thead class="bg-slate-100 text-xs text-slate-600 uppercase tracking-wider border-b border-slate-200">
                            <tr>
                                ...
                                <th class="px-4 py-2.5 font-bold">Delivery Method</th>
                                <th class="px-4 py-2.5 font-bold w-32">Status</th>
                                <th class="px-4 py-2.5 text-center font-bold w-40">Actions</th>
                            </tr>
                        </thead>
                        <!-- Body: Darkened row dividers to divide-slate-200 -->
                        <tbody class="divide-y divide-slate-200">
                            <!-- Rows: Changed hover to solid bg-slate-50 and adjusted inactive opacity, tightened padding -->
                            <tr v-for="method in deliverymethods.data" :key="method.id" class="transition-colors hover:bg-slate-50" :class="{'opacity-75 bg-slate-50': method.state === 0}">
                                <td class="px-4 py-2 font-mono text-sm text-slate-500">{{ method.id }}</td>
                                <td class="px-4 py-2 font-medium text-slate-800">{{ method.delivery_method_name }}</td>

                                <!-- Status Column -->
                                <td class="px-4 py-2">
                                    <span v-if="method.state === 1" class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-100">
                    ...
                    <!-- Empty State -->
                    <div v-if="deliverymethods.data.length === 0" class="py-12 text-center">
                        <div class="flex flex-col items-center">
                            ...
                            <h3 class="text-base font-semibold text-slate-900">No delivery methods found</h3>
                            <p class="mt-1 mb-5 text-sm text-slate-500 max-w-sm">
                                {{ searchTerm ? 'We couldn’t find anything matching your search. Try adjusting your keywords.' : 'Get started by creating your first delivery method (e.g., Courier, Hand-carried).' }}
                            </p>
                            <button
                                v-if="!searchTerm"
                                @click="openAddDialog"
                ...
                <div v-if="deliverymethods.last_page > 1" class="flex items-center justify-between border-t border-slate-200 bg-slate-50 px-4 py-3">
                    <p class="text-sm text-slate-500">
                        Showing <span class="font-medium text-slate-700">{{ deliverymethods.from }}</span>
                        to <span class="font-medium text-slate-700">{{ deliverymethods.to }}</span> of
                        <span class="font-medium text-slate-700">{{ deliverymethods.total }}</span> results
                    </p>
                    <div class="flex gap-1">
                        <button
                            v-for="(link, index) in deliverymethods.links.slice(1, -1)"
                            :key="index"
            ...
            <DeleteConfirmationModal
                ...
                title="Deactivate Delivery Method"
            />
                ...
                <div v-if="showDialog" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm px-4">
                    <div class="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-xl border border-slate-200">
                        ...
                        <h2 class="mb-6 pr-8 text-lg font-bold text-slate-800 tracking-tight">
                            {{ isEdit ? 'Edit Delivery Method' : 'Create Delivery Method' }}
                        </h2>
                        
                        <form @submit.prevent="submitForm" class="space-y-4">
                            <div>
                                <label class="mb-1.5 block text-sm font-medium text-slate-700" for="delivery_method_name">Method Name</label>
                                <input
                                    v-model="form.delivery_method_name"
                                    ...
                                    required
                                    placeholder="e.g. Courier, Hand-carried, Email"
                                />
                                <p v-if="form.errors.delivery_method_name" class="mt-1 text-xs text-red-500">{{ form.errors.delivery_method_name }}</p>
                            </div>
