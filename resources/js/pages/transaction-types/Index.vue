...
const clearSearch = () => {
    searchTerm.value = '';
    router.get(route('transaction-types.index'));
};
...
const breadcrumbs: BreadcrumbItem[] = [
    {
        title: 'Transaction Types Management',
...
// Open modal for new transaction type
...
function openEditDialog(transType: any) {
    isEdit.value = true;
    editingId.value = transType.id;
    form.name = transType.name;
    form.processing_days = transType.processing_days;
    form.state = transType.state;
    showDialog.value = true;
}

// Deactivate modal (Soft Delete)
function openDeleteModal(transType: { id: number; name: string }) {
    itemToDeleteUrl.value = route('transaction-types.destroy', transType.id);
    itemToDeleteName.value = transType.name;
    showDeleteModal.value = true;
}

// Quick Reactivate Action
function reactivateTransactionType(transType: any) {
    router.put(route('transaction-types.update', transType.id), {
        name: transType.name,
        processing_days: transType.processing_days,
...
function submitForm() {
    if (isEdit.value && editingId.value !== null) {
        form.put(route('transaction-types.update', editingId.value), {
            onSuccess: () => {
        ...
        form.post(route('transaction-types.store'), {
            onSuccess: () => {
                showDialog.value = false;
                form.reset();
            },
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex flex-col gap-6 p-6 w-full mx-auto bg-slate-50/50 min-h-screen">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-slate-800 tracking-tight">System Transaction Types</h1>
                    <p class="text-sm text-slate-500 mt-1">Manage global transaction categories and their SLA processing days.</p>
                </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.03)] overflow-hidden">
                ...
                <div v-if="transactiontypes.last_page > 1" class="flex items-center justify-between border-t border-slate-100 bg-slate-50/50 px-6 py-4">
                    <p class="text-sm text-slate-500">
                        ...
                        <span class="font-medium text-slate-700">{{ transactiontypes.total }}</span> results
                    </p>
                    <div class="flex gap-1">
                        <button
                            v-for="(link, index) in transactiontypes.links.slice(1, -1)"
                            :key="index"
            ...
            <DeleteConfirmationModal
                ...
                title="Deactivate Transaction Type"
            />
